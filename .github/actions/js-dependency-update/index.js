...
const infoBase = '[js-dependency-update] :'
...
async function run() {
  ...
  core.info(`${infoBase} base branch is ${baseBranch}`);
  core.info(`${infoBase} target branch is ${targetBranch}`);
  core.info(`${infoBase} working directory is ${workingDir}`);
  ...
  if (gitStatus.stdout.length > 0) {
    core.info(`${infoBase} There are updates available!`);
    ...
    try {
      ...
      core.error(`${infoBase} Something went wrong while creating the PR. Check logs below.`);
      core.setFailed(e.message);
      core.error(e);
    }
  } else {
    core.info(`${infoBase} No updates at this point in time.`);
  }
  ...
  4. If there are modified files
    [DONE] 1. Add and commit files to the target-branch
    [DONE] 2. Create a PR to the base-branch using the octokit API
